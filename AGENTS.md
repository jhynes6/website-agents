...
- **Crawl4AI needs Playwright Chromium**: The default crawler (`crawl4ai`) requires a full Chromium browser. After installing backend deps, run: `source backend/venv/bin/activate && playwright install chromium --with-deps`.
- **Multiple Next.js dev servers**: If the frontend port is busy (e.g. after a crash), Next.js silently starts on the next available port (3001, 3002...). Always kill stale `next-server` processes before restarting. Check with `netstat -tlnp | grep ':300'`.
- **Browser cache after dev server restart**: If the browser shows unstyled "Loading..." after a dev server restart, use Chrome DevTools "Empty Cache and Hard Reload" (right-click refresh button with DevTools open) to clear stale JS chunks.
