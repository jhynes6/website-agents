...
Validated on 1 Jul 2025 • Last edited on 17 Dec 2025
...
- You cannot bring your own models to use for agents or serverless inference when using the DigitalOcean Control Panel, `doctl` command-line interface, or DigitalOcean API. You can see the models we offer on our [model overview page](https://docs.digitalocean.com/products/gradient-ai-platform/details/models/).
  - **For agents built and deployed using Agent Development Kit**: You can use any model key, even if the model isn’t hosted on DigitalOcean or the model key is not provided by DigitalOcean.
- The following models are only available for [serverless inference](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/use-serverless-inference/) and not available for creating agents using the DigitalOcean Control Panel, `doctl` command-line interface, or DigitalOcean API:
  - [Alibaba Qwen3-32B model](https://huggingface.co/Qwen/Qwen3-32B)

  - [Multimodal models for image and audio generation provided by Fal](https://docs.digitalocean.com/products/gradient-ai-platform/details/models/#foundation-models) (in public preview)
...
- For agents built and deployed using Agent Development Kit (ADK):
  ...
  - Knowledge bases can be attached, detached, or managed only using [the knowledge base endpoints](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/build-agents-using-adk/#attach-knowledge-bases).

  - Agent deployments cannot be moved from one workspace to another. You must [redeploy the agent to a new workspace with the environment defined](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/build-agents-using-adk/#set-up-a-project).

  - Automatic rollback to a previous release is not available. You can [redeploy the agent code for that release to your environment](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/build-agents-using-adk/#set-up-a-project).

  - You can only [destroy an agent deployment using the DigitalOcean Control Panel](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/build-agents-using-adk/#destroy-an-agent-deployment).

## Knowledge Base Limits

- Retrieval requests are available only through the Knowledge Base endpoint at `https://kbaas.do-ai.run` and through the Gradient SDK. For guidance on using the retrieve API, see [Retrieve data from a knowledge base](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/create-manage-agent-knowledge-bases/#retrieve-api).

- Size estimates and indexable limits work differently depending on the data source:
  ...
  - Chunk sizes (`max_chunk_size`, `parent_chunk_size`, `child_chunk_size`) must remain within the token limits of the selected embedding model.
- All chunking strategies enforce a minimum chunk size of approximately 100 tokens.

- Chunking settings apply per data source, not globally. Knowledge base retrieval is available only through the DigitalOcean API in public preview.

- Changing the chunking strategy after a data source is created is not supported in public preview. To change strategies, you must [remove the data source](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/create-manage-agent-knowledge-bases/#remove-data-source-control-panel), and then [re-add it with the preferred configuration](https://docs.digitalocean.com/products/gradient-ai-platform/how-to/create-manage-agent-knowledge-bases/#add-data-source-control-panel).
